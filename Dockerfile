...
RUN apt-get update && \
    ...
    bash Miniforge3-Linux-x86_64.sh -b -p "/opt/mamba" && \
    rm Miniforge3-Linux-x86_64.sh && \
    echo "export PATH=/opt/mamba:$PATH" >> ~/.bashrc && \
    export PATH=/opt/mamba:$PATH && \
    micromamba shell init --shell bash --root-prefix=/opt/mamba &&\
